...
**API endpoints:**
- `POST /upload-csv` — Upload a CSV file (multipart form: `file` + `thread_id`)
- `GET /csv-metadata/{thread_id}` — Get metadata about uploaded CSVs for a session
- `DELETE /cleanup-csv/{thread_id}` — Delete all CSV data for a session
